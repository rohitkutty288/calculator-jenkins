...
<style>
  ...
  html,body{height:100%; margin:0; background:linear-gradient(180deg,#061026 0%, #081428 100%); display:flex; align-items:center; justify-content:center;}
  .wrap{width:360px; max-width:92vw; padding:28px; background:linear-gradient(180deg, rgba(255,255,255,0.02), rgba(255,255,255,0.01)); border-radius:18px; box-shadow:var(--shadow);}
  ...
  .display .small{position:absolute; left:24px; top:24px; color:#8aa0c7; font-size:12px;}
...
(function(){
  ...
  function percent(){
    try{
      const v = evalExpression(current) / 100;
      current = String(v);
      update();
    }catch(e){}
  }
  ...
  function evalExpression(expr){
    // replace unicode ops and guard common issues
    expr = expr.replace(/÷/g, '/').replace(/×/g, '*').replace(/−/g, '-');
    // basic safety: allow only numbers, operators, decimals, parentheses and spaces
    if(!/^[0-9+\-*/(). %]+$/.test(expr)) throw new Error('Bad expression');
    // evaluate safely using Function (still local!)
    return Function('"use strict"; return (' + expr + ')')();
  }

  function equals(){
    try{
      const result = evalExpression(current);
  ...
  document.addEventListener('keydown', (e)=>{
    if(e.key.match(/[0-9]/)) push(e.key);
      ...
      current = current.slice(0,-1) || '0'; update();
